...
- Fallback: a `deploy-api` CI job on pushes to `master` needs `check`, `image`, and `web`, then POSTs to `RENDER_DEPLOY_HOOK_URL`. It does nothing until that secret exists. With the secret set, `autoDeployTrigger` must be `off`.
